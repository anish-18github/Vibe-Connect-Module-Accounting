...
const CalculateBudget: React.FC = () => {
  ...
  const months = useMemo(() => getMonthsForPeriod(period as BudgetPeriod | ''), [period]);

  const incomeRows = useMemo(() => incomeAccounts, [incomeAccounts]);

  const expenseRows = useMemo(() => expenseAccounts, [expenseAccounts]);
  ...
  const accountRows = useMemo(() => [...incomeRows, ...expenseRows], [incomeRows, expenseRows]);
  ...
  const getAleBalance = (): number => getAssetTotal() - getLiabilityTotal() - getEquityTotal();
  ...
  const handleCellChange = (account: string, month: string, v: string) => {
  ...
  const getColumnTotal = (month: string): number =>
    accountRows.reduce((sum, acc) => sum + (values[acc]?.[month] || 0), 0);

  const getIncomeTotal = (): number => incomeRows.reduce((sum, acc) => sum + getRowTotal(acc), 0);

  const getExpenseTotal = (): number => expenseRows.reduce((sum, acc) => sum + getRowTotal(acc), 0);

  const getProfitOrLoss = (): number => getIncomeTotal() - getExpenseTotal();

  const getAssetTotal = (): number => assetRows.reduce((sum, acc) => sum + getRowTotal(acc), 0);

  const getLiabilityTotal = (): number =>
    liabilityRows.reduce((sum, acc) => sum + getRowTotal(acc), 0);

  const getEquityTotal = (): number => equityRows.reduce((sum, acc) => sum + getRowTotal(acc), 0);
        ...
        <div className="item-card-body">
          <div className="table-responsive">
            <table className="table table-sm align-middle item-table-inner">
              <thead>
                <tr>
                  <th className="fw-medium text-dark" style={{ minWidth: 220 }}>
                    Account
                  </th>
                  {months.map((m) => (
                    <th key={m} className="fw-medium text-dark text-end" style={{ width: 110 }}>
                      {m}
                    </th>
                  ))}
                  <th className="fw-medium text-dark text-end" style={{ width: 110 }}>
                    Total
                  </th>
                </tr>
              </thead>
                ...
                {incomeRows.length > 0 && (
                  <>
                    ...
                    {showIncomeRows &&
                      incomeRows.map((acc) => (
                        <tr key={`income-${acc}`}>
                          ...
                          <td className="text-end fw-semibold align-middle">{getRowTotal(acc)}</td>
                        </tr>
                      ))}
                    ...
                    <tr className="table-light">
                      ...
                      {months.map((m) => (
                        <td key={m} className="text-end align-middle">
                          {incomeRows.reduce((sum, acc) => sum + (values[acc]?.[m] || 0), 0)}
                        </td>
                      ))}
                      <td className="text-end fw-semibold align-middle">{getIncomeTotal()}</td>
                    </tr>
                  </>
                )}
                ...
                {expenseRows.length > 0 && (
                  <>
                    ...
                    <tr className="table-light">
                      ...
                      {months.map((m) => (
                        <td key={m} className="text-end align-middle">
                          {expenseRows.reduce((sum, acc) => sum + (values[acc]?.[m] || 0), 0)}
                        </td>
                      ))}
                      <td className="text-end fw-semibold align-middle">{getExpenseTotal()}</td>
                    </tr>
                  </>
                )}
              </tbody>
                  ...
                  {showAssetRows &&
                    assetRows.map((acc) => (
                      <tr key={`asset-${acc}`}>
                        ...
                        <td className="text-end fw-semibold">{getRowTotal(acc)}</td>
                      </tr>
                    ))}
                  ...
                  <tr className="table-light">
                    ...
                    {months.map((m) => (
                      <td key={m} className="text-end">
                        {assetRows.reduce((sum, acc) => sum + (values[acc]?.[m] || 0), 0)}
                      </td>
                    ))}
              ...
              {liabilityRows.length > 0 && (
                <>
                  ...
                  <tr className="table-light">
                    ...
                    {months.map((m) => (
                      <td key={m} className="text-end">
                        {liabilityRows.reduce((sum, acc) => sum + (values[acc]?.[m] || 0), 0)}
                      </td>
                    ))}
                    <td className="text-end fw-semibold">{getLiabilityTotal()}</td>
                  </tr>
                </>
              )}
              ...
              {equityRows.length > 0 && (
                <>
                  ...
                  <tr className="table-light">
                    ...
                    {months.map((m) => (
                      <td key={m} className="text-end">
                        {equityRows.reduce((sum, acc) => sum + (values[acc]?.[m] || 0), 0)}
                      </td>
                    ))}
