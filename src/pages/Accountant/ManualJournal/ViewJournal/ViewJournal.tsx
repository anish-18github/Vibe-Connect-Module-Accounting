...
export default function ViewJournal() {
  ...
  const handleDownloadPdf = () => {
    ...
    doc.text(`Journal: ${journal.journal}`, 14, y);
    y += 7;
    doc.text(`Reference No: ${journal.referenceNo}`, 14, y);
    y += 7;
    doc.text(`Status: ${journal.status}`, 14, y);
    y += 7;
    doc.text(`Date: ${journal.createdOn}`, 14, y);
    y += 7;
    doc.text(`Created By: ${journal.createdBy}`, 14, y);
    y += 10;

    doc.text(`Amount: ${journal.amount} ${journal.currency}`, 14, y);
    y += 10;
        ...
        >
          ...
          <div className="d-flex justify-content-between mb-3">
            <div>
              ...
              <div className="small text-muted">Journal #{journal.referenceNo}</div>
            </div>
          ...
          <div className="row small mb-3">
            <div className="col-sm-6 mb-1">
              ...
              {journal.amount.toFixed ? journal.amount.toFixed(2) : journal.amount}{' '}
              {journal.currency}
            </div>
          </div>
          ...
          <div className="small">
            <strong>Notes</strong>
            <div className="mt-1 text-muted">{journal.notes || '-'}</div>
          </div>
        </div>
      </div>
    </>
